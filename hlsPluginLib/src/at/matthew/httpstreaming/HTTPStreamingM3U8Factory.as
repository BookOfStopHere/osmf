...
package at.matthew.httpstreaming
{
	...
	public class HTTPStreamingM3U8Factory extends HTTPStreamingFactory
	{
		...
		{
			// nweber - add MBR support
			var dsResource:DynamicStreamingResource = (resource as DynamicStreamingResource)
			if (dsResource != null) {
				var items:Vector.<HTTPStreamingPlaylist> = new Vector.<HTTPStreamingPlaylist>();
				for each (var dsItem:DynamicStreamingItem in dsResource.streamItems) {
					items.push(new HTTPStreamingPlaylist(dsItem.streamName, dsItem.bitrate));
				}
				return new HTTPStreamingItemsIndexInfo(dsResource.url, items);
			}
			// original
			else {
