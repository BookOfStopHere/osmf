 ...
 package at.matthew.httpstreaming
{
	...
	public class HTTPStreamingM3U8IndexHandler extends HTTPStreamingIndexHandlerBase
	{
		...
		override public function initialize(indexInfo:Object):void
		{
			...
			// nweber - DVR support
			else if(indexInfo is HTTPStreamingItemsIndexInfo)
			{
				_indexString = (indexInfo as HTTPStreamingItemsIndexInfo).url;
				
				var streamQualityRates:Array = [];
				var streamNames:Array = [];
				
				var streamInfos:Vector.<HTTPStreamingPlaylist> = (indexInfo as HTTPStreamingItemsIndexInfo).streamInfos;
				var streamCount:int = streamInfos.length;
				
				for (var quality:int = 0; quality < streamCount; quality++)
				{
					streamQualityRates[quality]	= streamInfos[quality].bitrate;
					streamNames[quality] = streamInfos[quality].streamName;
				}
		...
		override public function processIndexData(data:*, indexContext:Object):void
		{
			...
			// TODO : Support DVR
			/*
			// hack to support duration properly
			var scriptData:FLVTagScriptDataObject = new FLVTagScriptDataObject;				
			scriptData.objects = [ "onMetaData", { "duration": programDuration } ];
			dispatchEvent(new HTTPStreamingEvent(HTTPStreamingEvent.SCRIPT_DATA, false, false, NaN, scriptData, FLVTagScriptDataMode.IMMEDIATE));
			*/
			
			/*
			// manually send off a DVR info
			if ((indexContext as HTTPStreamingM3U8IndexRateItem).live) {
				m_dvrInfo = new DVRInfo;
				m_dvrInfo.isRecording = 
				m_dvrInfo.beginOffset = 
				m_dvrInfo.startTime = 0;
				m_dvrInfo.endOffset = 
				m_dvrInfo.curLength = 
				m_dvrInfo.offline = 
				m_dvrInfo.windowDuration = 
				
				dispatchEvent(new DVRStreamInfoEvent(DVRStreamInfoEvent.DVRSTREAMINFO, false, false, m_dvrInfo));
			}
			*/
		}
